...
@section('content')
    ...
    <header class="site-header" id="siteHeader">
        <div style="display: flex; align-items: center; gap: 12px;">
            <img src="{{ $logoUrl }}" alt="Logo INTERPRIMA INDOCOM" style="height: 38px; width: auto; object-fit: contain;" />
            ...
            <div class="filter-bar" role="tablist" aria-label="Filter berdasarkan Brand">
                @foreach($brands as $index => $brand)
                    <button 
                        ...
                        onclick="filterBrand('{{ $brand->slug }}', this)"
                    >
                        {{ $brand->name }}
                    </button>
                @endforeach
    ...
    {{-- ============================
         PRODUCT SECTIONS (GROUPED BY BRAND & PRODUCT GROUP)
         ============================ --}}
    @if($products->count() > 0)
        @php
        ...
        <div id="catalogGridContainer" style="max-width: 1400px; margin: 0 auto;">
            @foreach($brands as $brand)
                ...
                @endphp
                <div class="brand-section {{ $isFirstBrand ? '' : 'hidden' }}" 
                     data-brand="{{ $brand->slug }}"
                     style="opacity: {{ $isFirstBrand ? '1' : '0' }}; transform: scale({{ $isFirstBrand ? '1' : '0.98' }}); transition: opacity 300ms ease, transform 300ms ease;">
                    
                    @foreach($brand->productGroups as $group)
                        @php
                            ...
                            <div class="group-wrapper" style="margin-bottom: 48px; padding: 0 24px;">
                                ...
                                <main class="catalog-grid" style="padding: 0 0 20px;">
                                    @foreach($groupProducts as $product)
                                        <article
                                            class="product-card {{ $isFirstBrand ? '' : 'hidden' }}"
                                            data-index="{{ $globalCardIndex++ }}"
                                            data-brand="{{ $brand->slug }}"
                                            data-name="{{ $product->name }}"
                                            data-brand-name="{{ $brand->name }}"
                                            data-description="{{ $product->description ?? '' }}"
                                            data-is-pdf="{{ $product->isPdf() ? 'true' : 'false' }}"
                                            data-show-description="{{ $product->show_description ? 'true' : 'false' }}"
                                            data-url="{{ $product->image_url }}"
                                            data-thumbnail-url="{{ $product->thumbnail_url }}"
                                            data-pdf-pages="{{ json_encode($product->pdf_pages_urls) }}"
                                            onclick="openLightbox('{{ $product->image_url }}', this)"
                                            role="button"
                                            tabindex="0"
                                        ...
                                        >
                                            ...
                                            <div class="product-card__info">
                                                <span class="product-card__brand">{{ $brand->name }}</span>
                                                <h3 class="product-card__name">{{ $product->name }}</h3>
                                                @if($product->description && $product->show_description && !$product->isPdf())
                                                    <p class="product-card__description">{{ Str::limit($product->description, 100) }}</p>
                                                @endif
                                            </div>
                                        </article>
                                    @endforeach
                    ...
                    @if($ungroupedProducts->count() > 0)
                        <div class="group-wrapper" style="margin-bottom: 48px; padding: 0 24px;">
                            @if($brand->productGroups->count() > 0)
                                <h2 class="group-title" style="font-family: var(--font-display); border-bottom: 2px solid rgba(22, 101, 52, 0.1); padding-bottom: 8px; margin-bottom: 24px; color: #1e293b; font-size: 1.125rem; font-weight: 700; letter-spacing: 0.05em; display: flex; align-items: center; gap: 8px;">
                                    Others
                                </h2>
                            @endif
    ...
    <footer class="site-footer">
        <p class="site-footer__text">&copy; {{ date('Y') }} PT. Interprima Indocom. All rights reserved.</p>
    </footer>
@endsection
